...
export default function AudioPlayer({ meetingId }: { meetingId: string }) {
  ...
  if (!url) return (
    <div className="fixed bottom-0 left-0 md:left-[240px] right-0 bg-white border-t border-gray-100 px-4 py-3 text-sm text-gray-400">
      Loading audio...
    </div>
  );

  return (
    <div className="fixed bottom-0 left-0 md:left-[240px] right-0 bg-white border-t border-gray-100 px-4 py-3 flex flex-col gap-2 z-10">
      <audio
        ref={audioRef}
        src={url}
      ...
      <div className="flex items-center gap-3">
        <span className="text-xs text-gray-500 tabular-nums shrink-0">{fmt(currentTime)} / {fmt(duration)}</span>
